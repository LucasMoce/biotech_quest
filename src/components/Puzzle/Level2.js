...
const Puzzle1 = ({aumentarPontos, puzzleLevelsCheck}) => {
  const [circleCadeias, setCircleCadeias] = useState([
    ...
    [
      ...
      { x: 875, y: 250, label: 'T' },
      { x: 975, y: 250, label: 'C' },
      { x: 1075, y: 250, label: 'C' },
    ],
    [
      { x: 825, y: 325, label: 'A' },
      { x: 925, y: 325, label: 'T' },
      { x: 1025, y: 325, label: 'G' },
    ],
    [
      { x: 825, y: 400, label: 'C' },
      { x: 925, y: 400, label: 'A' },
      { x: 1025, y: 400, label: 'T' },
    ],
  ]);
  ...
  const encaixes = [
    ...
    { x: 825, y: 550, label: 'C' },
    { x: 925, y: 550, label: 'C' },
    { x: 1025, y: 550, label: 'A' },
    { x: 1125, y: 550, label: 'T' },
    { x: 1225, y: 550, label: 'C' },
  ...
  const checkAnswer = () => {
    ...
    if (encaixesOcupados && circulosCorretos) {
      ...
      if  (!puzzleLevelsCheck[2]) {
        {aumentarPontos(10)}
        puzzleLevelsCheck[2] = true
      }
      } 
  ...
  return (
    <div>
      ...
      <div className="justify-content-center mt-3 text-center">
        ...
        <Link to={nextLvl} className={puzzleLevelsCheck[2] ? "btn btn-info ml-2" : "disabled btn btn-info ml-2"}>
          Próximo Nível
        </Link>
      </div>

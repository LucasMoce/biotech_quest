...
const Puzzle1 = ({aumentarPontos, puzzleLevelsCheck}) => {
  ...
  const checkAnswer = () => {
    ...
    if (encaixesOcupados && circulosCorretos) {
      ...
      if  (!puzzleLevelsCheck[6]) {
        {aumentarPontos(10)}
        puzzleLevelsCheck[6] = true
      }
      } 
    else {
  ...
  return (
    <div>
      ...
      <div className="justify-content-center mt-3 text-center">
        ...
        <Link to={nextLvl} className={puzzleLevelsCheck[6] ? "btn btn-info ml-2" : "disabled btn btn-info ml-2"}>
          Próximo Nível
        </Link>
      </div>
